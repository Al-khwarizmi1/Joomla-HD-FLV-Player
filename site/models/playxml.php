...
class hdflvplayerModelplayxml extends HdflvplayerModel {
    ...
    function playgetrecords() {
        ...
        $moduleid           = intVal(JRequest::getvar('mid', '', 'get', 'int'));
        $playlistid         = intVal(JRequest::getvar('playid', '', 'get', 'var'));
        $videoid            = intVal(JRequest::getvar('id', '', 'get', 'int'));
        $compid             = intVal(JRequest::getvar('compid', '', 'get', 'int'));

        ## Getting necessary inputs from URL if Joomla version is 3.0
        if (version_compare(JVERSION, '3.0.0', 'ge')) {
            $moduleid       = intVal(JRequest::getvar('mid'));
            $playlistid     = intVal(JRequest::getvar('playid'));
            $videoid        = intVal(JRequest::getvar('id'));
        ...
        if ($moduleid != 0) {
            $moduleid = $moduleid;
            $query = 'SELECT params FROM `#__modules` WHERE id=' . intval($moduleid);
            $db->setQuery($query);
        ...
        if ($videoid != '') {
            ...
            $query = 'SELECT a.`id`, a.`title`, a.`filepath`, a.`times_viewed`, a.`videourl`,a.`thumburl`, a.`previewurl`, a.`hdurl`,
                    ...
                    WHERE a.published=1 AND a.id=' . intval($videoid);
            $db->setQuery($query);
            ...
            if (count($rows) > 0) {
                $query = 'SELECT a.`id`, a.`title`, a.`filepath`, a.`times_viewed`, a.`videourl`,a.`thumburl`, a.`previewurl`, a.`hdurl`,
                        ...
                        LEFT JOIN #__hdflvplayerads AS c ON a.prerollid = c.id
                        LEFT JOIN #__hdflvplayerads AS d ON a.postrollid = d.id
                        WHERE a.published=1 AND a.id !=' . intval($videoid) . ' AND b.id=' . $rows[0]->playlistid;
                $db->setQuery($query);
        ...
        ## if playlist id exist, fetch the vidoes and the playlist detail
            ...
            if ($playlistid == 0) {
                ...
                $where = ' AND a.playlistid=' . intval($playlistid) . ' ORDER BY ordering ASC';
            }
            $query = 'SELECT a.`id`,  a.`title`, a.`filepath`, a.`times_viewed`, a.`videourl`, a.`thumburl`, a.`previewurl`, a.`hdurl`,
                    a.`playlistid`,a.`streamerpath`, a.`streameroption`, a.`postrollads`, a.`prerollads`, a.`midrollads`,a.`imaads`,
                    a.`description`,a.`targeturl`, a.`download`, a.`prerollid`, a.`postrollid`, a.`access`, a.`islive`,
