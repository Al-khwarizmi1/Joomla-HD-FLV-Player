...
class hdflvplayerModelplayer extends HdflvplayerModel
{
	...
	function showhdplayer()
	{
		...
		$playid             = intVal(JRequest::getvar('id','','get','var'));
                
                ## Fetch Playlist ID from URL
		$compid             = intVal(JRequest::getvar('compid','','get','int'));
                }else{
                  $playid           = intVal(JRequest::getvar('id'));

                  ## Fetch Playlist ID from URL
		$compid             = intVal(JRequest::getvar('compid'));
                }
		if($compid != '')
		{
			$playlistid = $compid;
		...
		if($playid!='')
		{
		 $query             = 'SELECT `id`, `title`, `times_viewed`, `filepath`, `videourl`, `thumburl`, `previewurl`, `hdurl`,
                                    ...
                                    `targeturl`, `download`, `prerollid`, `postrollid`, `access`, `islive` FROM `#__hdflvplayerupload`
                                     WHERE published=1 AND id='.intval($playid).' ORDER BY ordering ASC ';
		$db->setQuery( $query );
		$rows               = $db->loadObject();
		...
		$query              = 'SELECT `id`, `title`, `times_viewed`, `filepath`, `videourl`, `thumburl`, `previewurl`,
                                    ...
                                    `prerollads`, `midrollads`, `description`, `targeturl`, `download`, `prerollid`, `postrollid`, `access`, `islive`
                                     ...
                                     WHERE published=1 AND playlistid='.intval($playlistid).' ORDER BY ordering ASC';
		$db->setQuery( $query );
		$rows               = $db->loadObject();
		}
		...
		if (!empty($rows)) {
			...
			$query      = 'UPDATE #__hdflvplayerupload SET times_viewed=1+times_viewed WHERE id='.intval($playid);
			$db->setQuery($query );
			$db->query();
			$playid     = $rows->id;
		}
		...
		if($playlistid != '' && $playlistid != 0 )
		{
                 ...
                 if($playid!='')
                 {
                     $pwhere        = ' AND id NOT IN ('.intval($playid).')';
                 }
                 $query             = 'SELECT count(`id`) FROM `#__hdflvplayerupload`
                                        WHERE published=1 AND playlistid='.intval($playlistid). $pwhere.'  ORDER BY ordering ASC';
                $db->setQuery( $query );
		...
		{
                $query              = 'SELECT `playlistid` FROM `#__hdflvplayerupload` WHERE published=1 and id='.intval($playid);
		$db->setQuery( $query );
		$playlistId_video   = $db->loadResult();
                
                $limit_query        = 'SELECT count(`id`) FROM `#__hdflvplayerupload` WHERE published=1 AND id NOT IN ('.intval($playid).') AND playlistid='.intval($playlistId_video).' ORDER BY ordering ASC ';

                 $query1             = 'SELECT `id`, `title`, `times_viewed`, `filepath`, `videourl`, `thumburl`, `previewurl`,
                                    `hdurl`, `home`, `playlistid`, `streamerpath`, `streameroption`, `postrollads`,
                                    `prerollads`, `midrollads`, `description`, `targeturl`, `download`, `prerollid`, `postrollid`, `access`,
                                    `islive` FROM `#__hdflvplayerupload` WHERE published=1 AND id NOT IN ('.intval($playid).') AND playlistid='.intval($playlistId_video).'
		...
		$pageno = 1;
                ...
                if(!version_compare(JVERSION, '3.0.0', 'ge')) {
                    $page = intVal(JRequest::getvar('page','','get','int'));
                }else{
