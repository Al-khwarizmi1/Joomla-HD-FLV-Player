...
class hdflvplayerModelshowads extends HdflvplayerModel {
	...
	function __construct()
	{
		...
		$this->setState('limitstart', intVal(JRequest::getVar('limitstart', 0, '', 'int')));
	}
...
//		$lists['adsname'] = JHTML::_('list.category',  'filter_playlistname', 'com_hdflvplayer', (int) $rs_showadsname, $javascript );
	...
	function saveads($task) {
		...
		$cid = strip_tags(JRequest::getVar('cid', array(0), '', 'array'));
		$id = $cid[0];
		$adsSave->load($id);
                $adsetting = JRequest::get('post');
                $typeofadd = strip_tags(JRequest::getVar('typeofadd','post'));
                
                ## Get IMA ad details and serialize data
                if($typeofadd=="ima"){
                $imaaddetail                    = array(
                    'imaadtype'                 => strip_tags($adsetting['imaadtype']),
                    'videoimaadwidth'           => strip_tags($adsetting['videoimaadwidth']),
                    'videoimaadheight'          => strip_tags($adsetting['videoimaadheight']),
                    'publisherId'           => strip_tags($adsetting['publisherId']),
                    'contentId'           => strip_tags($adsetting['contentId']),
                    'channels'           => strip_tags($adsetting['channels']),
                    'imaadpath'           => strip_tags($adsetting['imaadpath'])
                );
		...
		$adsSave->adsdesc = strip_tags(JRequest::getVar('adsdesc', '', 'post', 'string', JREQUEST_ALLOWRAW));
		$adsSave->adsname = strip_tags(JRequest::getVar('adsname', '', 'post', 'string', JREQUEST_ALLOWRAW));

		//Stores the input into table into appropriate columns
		$fileoption = strip_tags(JRequest::getVar('fileoption','post'));
		if (!$adsSave->store()) {
			JError::raiseError(500, JText::_($adsSave->getError()));
		}
		...
		if ($fileoption == 'Url') {
			$postvideopath = strip_tags(JRequest::getVar('posturl-value','post'));
		...
		//If File Path option File means, the below code will work.
			...
			$normal_video = strip_tags(JRequest::getVar('normalvideoform-value','post'));
