...
class hdflvplayerModeluploadvideos extends HdflvplayerModel {
	...
	function videoslist()
	{
		...
		$search = strip_tags(JRequest::getVar('search', '', 'string'));
	...
	function setdefault()
	{
		...
		$db =& JFactory::getDBO();
		$cid	= strip_tags(JRequest::getVar( 'cid', array(), 'post', 'array' ));//getting selected record
	...
	{
		$db = JFactory::getDBO();
		$listitem = strip_tags(JRequest::getvar('listItem','','get','var'));
